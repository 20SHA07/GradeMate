import type { ExtractedSyllabus } from "./extractSyllabus";

export type SyllabusFewShotExample = {
  id: string;
  department: string;
...
export function selectFewShotExamples(inputText: string, courseCode?: string | null) {
  const department =
    getDepartment(courseCode ?? undefined) ?? getDepartment(inputText) ?? null;
...
export function formatFewShotExamplesForPrompt(inputText: string, courseCode?: string | null) {
  const examples = selectFewShotExamples(inputText, courseCode);
...
function getDepartment(value?: string | null) {
